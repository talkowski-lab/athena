...
def _split_pairtopair_by_binpairs(track_hits, pairs_bedpe_bt, counts_only=False):
    ...
    if counts_only:
        ...
        starting_value = ""
    split_res = {
        "_".join([pair[0], str(pair[1]), str(pair[5])]): starting_value
    ...
    if not counts_only:
        split_res = {
            tn: pbt.BedTool(h, from_string=True) for tn, h in split_res.items()
        }
...
def _bam_to_bedpe(bam_path, query_regions=None, clean_nsorted_copy=True):
    ...
    bedpe = (
        pbt.BedTool(sorted_path).bam_to_bed(bedpe=True).sort().cut(range(6)).saveas()
    )
...
def add_pairwise_bedtool_track(pairs_bedpe_bt, track, action, binsize):
    ...
    if action in "count-pairs pairwise-coverage any-pairwise-overlap".split():
        ...
        if action in "count-pairs any-pairwise-overlap".split():
            ...
            counts_only = False
        hits_per_bin = _split_pairtopair_by_binpairs(
            track_hits, pairs_bedpe_bt, counts_only=counts_only
        )
...
def add_pairwise_local_track(
    pairs_bedpe_bt, track, action, query_regions, binsize, quiet
):
    """
    ...
    if quiet is False:
        status_msg = (
            '[{0}] athena annotate-pairs: Adding track "{1}" ' + 'with action "{2}"'
        )
        print(
            status_msg.format(
                datetime.now().strftime("%b %d %Y @ %H:%M:%S"), track, action
            )
        )
...
def add_pairwise_ucsc_track(
    ...
    if ucsc.table_exists(db, table):
        status_msg = (
            "[{0}] athena annotate-pairs: Adding UCSC track "
            + '"{1}" with action "{2}"'
        )
        print(
            status_msg.format(
                datetime.now().strftime("%b %d %Y @ %H:%M:%S"), table, action
            )
        )
        if chromsplit:
            contigs = chromsort(
                list(
                    set(
                        [
                            f.chrom
                            for f in pairs_bedpe_bt.each(ucsc._check_hg_compliance)
                        ]
                    )
                )
            )
            ...
            if isinstance(sub_ures[0], pbt.BedTool):
                ...
                err = (
                    "QUERY RESULTS ERROR: Unknown query return format from "
                    + 'table "{0}" for reference "{1}"'
                )
                exit(err.format(table, ucsc_ref))
        else:
            ures = ucsc.query_ucsc(
                pairs_bedpe_bt, table, columns, conditions, db, action, query_regions
            )
    else:
        from sys import exit
...
def annotate_pairs(
    ...
    allbins_bt = (
        split_pair_bts[0].cat(*split_pair_bts[1:], postmerge=False).sort().merge(d=-1)
    )
    ...
    if len(tracks) > 0:
        for track in tracks:
            ...
            pairs_df["newtrack_{}".format(track_counter)] = add_pairwise_local_track(
                pairs_bedpe_bt, track, action, query_regions, binsize, quiet
            )
            track_counter += 1
    ...
    if len(ucsc_tracks) > 0:
        if quiet is False:
            status_msg = (
                "[{0}] athena annotate-pairs: Connecting to UCSC "
                + "Genome Browser database"
            )
            print(
                status_msg.format(datetime.now().strftime("%b %d %Y @ %H:%M:%S"), fasta)
            )
        ...
        for track in ucsc_tracks:
            action = actions[track_counter]
            pairs_df["newtrack_{}".format(track_counter)] = add_pairwise_ucsc_track(
                pairs_bedpe_bt,
                db,
                track,
                action,
    ...
    if fasta is not None:
        ...
        if quiet is False:
            status_msg = (
                "[{0}] athena annotate-pairs: Adding sequence homology "
        ...
        for identity in homology_cutoffs:
            for rev in True, False:
                pairs_df["newtrack_{}".format(track_counter)] = add_homology(
                    pairs_bt, fasta, binsize, identity, rev
                )
                track_counter += 1
